...
If you are using any kind of AI assistance while contributing to `xclim`, **this must be disclosed in the pull request**, along with the extent to which AI assistance was used (e.g. docs only vs. code generation).
If Pull Request responses are being generated by artificial intelligence agents, we ask that you disclose that as well.
...
Failing to disclose the use of artificial intelligence models, tools, or services is not respectful to the developers and maintainers of software projects.
...
Whenever possible, contributors must ensure that they are not reproducing code snippets that have been released under an incompatible license.
Code generated by artificial intelligence models can sometimes leak training inputs and reasonable actions should be taken to ensure that generated outputs are not reproductions of existing code.
