...
def parse_offset(freq: str) -> Sequence[str]:
    ...
    is_start_anchored : bool
      ...
      Can only be False when base is Y, Q or M; in other words, xclim assumes frequencies finer than monthly are all start-anchored.
    anchor : str or None
      Anchor date for bases Y or Q. As xarray doesn't support "W", neither does xclim (anchor information is lost when given).
...
def construct_offset(mult: int, base: str, start_anchored: bool, anchor: str | None):
    ...
    start_anchored: bool
      If base in [Y, A, Q, M] : True adds the "S" flag, False adds 'E'.
    anchor: str, optional
...
def stack_periods(
    ...
    It is reversible for some cases (see `stride`). A rolling-construct method will be much more performant for uniform periods (days, weeks).
...
def unstack_periods(da: xr.DataArray | xr.Dataset, dim: str = "period"):
    ...
    The row index ``i``  the period index in the stacked dataset, columns are the stride-long section of the original
    timeseries.
    ...
    def _reconstruct_time(start):
        times = time_as_delta + start
