...
def compute_bootstrapped_exceedance_rate(da: DataArray, config: BootstrapConfig) -> DataArray:
    ...
    in_base_exceedance_rates = _bootstrap_period(in_base_period, config)
...
def _bootstrap_year(ds_in_base_period: DataArray,
                    out_base_year: int,
                    config: BootstrapConfig) -> DataArray:
    ...
    for year_ds in in_base.groupby("time.year"):
        ...
        exceedance_rate = _calculate_exceedances(thresholds, out_base, config)
...
def _calculate_exceedances(in_base_percentiles: DataArray,
                           out_base: DataArray,
                           config: BootstrapConfig) -> DataArray:
    thresholds = convert_units_to(in_base_percentiles, out_base)
    thresholds = resample_doy(thresholds, out_base)
    exceedances = threshold_count(out_base, config.operator, thresholds, 'D')
    return to_agg_units(exceedances, thresholds, "count")
