...
class TestAgroclimaticIndices:
    ...
    # The values for the test below are obtained by using the `climate_indices` library
    # from monocongo. `climate_indices` uses a different method to obtain fit parameters.
    # If `xclim` is modified to use the same method (which amounts to only take the
    # approximated parameters generated by `_fit_start`), a good agreement is found for
    # the SPI indices. However, in the test below, we compare the results with the
    # different fitting methods. For this reason, a tolerance on the difference is
    ...
    @pytest.mark.parametrize(
        "freq, window, dist, method,  values, diff_tol",
        [
            ("MS", 1, "gamma", "APP", [1.375, 1.578, 2.103, -3.09, 0.868], 0.001),
            ("MS", 12, "gamma", "APP", [0.624, 1.648, 1.800, 0.986, 0.671], 0.001),
            ("MS", 1, "gamma", "ML", [1.375, 1.578, 2.103, -3.09, 0.868], 0.1),
            ("MS", 12, "gamma", "ML", [0.624, 1.648, 1.800, 0.986, 0.671], 0.075),
            ("MS", 1, "fisk", "ML", [1.417, 1.512, 1.982, -3.09, 0.943], 0.005),
            ("MS", 1, "fisk", "ML", [1.73, 1.51, 2.05, -3.09, 0.892], 0.35),
            ("MS", 12, "fisk", "ML", [0.709, 1.551, 1.670, 1.022, 0.701], 0.005),
            ("MS", 12, "fisk", "ML", [0.712, 1.53, 1.65, 1.02, 0.722], 0.1),
        ],
    )
