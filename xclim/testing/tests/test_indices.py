...
class TestAgroclimaticIndices:
    ...
    # The values for the test below are obtained by using the `climate_indices` library from monocongo.
    # `climate_indices` uses a different method to obtain fit parameters. If `xclim` is modified to use the same method
    # (which amounts to only take the approximated parameters generated by `_fit_start`), a good agreement
    # is found for the SPI indices. However, in the test below, we compare the results with the different fitting methods.
    # For this reason, a tolerance on the difference is supplied for each sample to be tested. This tolerance
    # can get quite large as the dataset only includes 4 years, i.e. different fitting methods are compared for data with 4 points.
    @pytest.mark.parametrize(
