...
def rle(
    ...
    if lastday:
        # forward-fill nans with last position before
        z = b.ffill(dim=dim)
    else
    ...
    if lastday:
        d = d.where(d >= 0)[1:,:,:]
        # fudge the dates back to where they started (padding & differencing has put them out by 1)
        d = d.assign_coords(time = da.time.values)
    else:
        d = d.where(d >= 0)
