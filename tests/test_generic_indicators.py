...
class TestFit:
    def test_simple(self, pr_ndseries, random, as_da):
    ...
    def test_nan(self, pr_series, random, as_da):
        r = random.random(22)
        r[0] = np.nan
        pr = pr_series(r)
    ...
    def test_ndim(self, pr_ndseries, random, as_da):
        pr = pr_ndseries(random.random((100, 1, 2)))
        out = generic.fit(pr, dist="norm")
        assert out.shape == (2, 1, 2)
        np.testing.assert_array_equal(out.isnull(), False)

    def test_options(self, rivo_series, random, as_da):
...
class TestReturnLevel:
    def test_seasonal(self, ndrivo_series, as_da):
        out = generic.return_level(ndrivo_series, mode="max", t=[2, 5], dist="gamma", season="DJF")
    ...
    def test_any_variable(self, pr_series, random, as_da):
        pr = pr_series(random.random(100))
        out = generic.return_level(pr, mode="max", t=2, dist="gamma")
        assert out.units == pr.units

    def test_no_indexer(self, ndrivo_series, as_da):
        out = generic.return_level(ndrivo_series, mode="max", t=[2, 5], dist="gamma")
    ...
    def test_q27(self, ndrivo_series, as_da):
        out = generic.return_level(ndrivo_series, mode="max", t=2, dist="gamma", window=7)
        assert out.shape == (1, 2, 3)

    def test_empty(self, ndrivo_series, as_da):
...
class TestStatistics:
    ...
    def test_simple(self, pr_series, random, op, word, as_da):
        pr = pr_series(random.random(400))
        out = generic.statistics(pr, freq="YS", statistic=op)
        assert out.long_name == f"{word} of variable"

    def test_ndq(self, ndrivo_series, as_da):
        out = generic.statistics(ndrivo_series, freq="YS", statistic="min", season="MAM")
        assert out.attrs["units"] == "m3 s-1"

    def test_missing(self, ndrivo_series, as_da):
        a = ndrivo_series.where(~((ndrivo_series.time.dt.dayofyear == 5) & (ndrivo_series.time.dt.year == 1902)))
    ...
    def test_3hourly(self, pr_hr_series, random, as_da):
