...
def test_saturation_vapor_pressure(tas_series):
    ...
    e_sat = convert.saturation_vapor_pressure(
        tas=tas,
        method="sonntag90",
        ice_thresh="0 degC",
    )
...
def test_relative_humidity(tas_series, hurs_series, huss_series, ps_series):
    ...
    hurs = convert.relative_humidity(
        tas=tas,
        huss=huss,
        ps=ps,
        method="sonntag90",
...
def test_specific_humidity(tas_series, hurs_series, huss_series, ps_series):
    ...
    huss = convert.specific_humidity(
        tas=tas,
        hurs=hurs,
        ps=ps,
        method="sonntag90",
...
def test_specific_humidity_from_dewpoint(tas_series, ps_series, huss_series):
    ...
    huss = convert.specific_humidity_from_dewpoint(
        tdps=tdps,
        ps=ps,
        method="sonntag90",
    )
...
def test_snowfall_approximation(pr_series, tasmax_series):
    ...
    prsn = convert.snowfall_approximation(pr, tas=tasmax, thresh="5 degC", method="binary")
...
class TestMeanRadiantTemperature:
    def test_mean_radiant_temperature(self, atmosds):
        ...
        mrt_sun = convert.mean_radiant_temperature(rsds, rsus, rlds, rlus, stat="sunlit")
