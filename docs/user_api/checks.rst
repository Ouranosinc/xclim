...
Health Checks
=============
The :class:`Indicator` class performs a number of sanity checks on inputs to make sure valid data is fed to the compute functions (:py:mod:`~xclim.core.cfchecks` for checks on the metadata and :py:mod:`~xclim.core.datachecks` for checks on the coordinates).
