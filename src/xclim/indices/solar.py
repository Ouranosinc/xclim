...
def _solar_noon_internal(ds: xr.Dataset | xr.DataArray) -> xr.DataArray:
    ...
    accuracy is only correct on the order of ±180s, and gets worse in the latter half of the 21st century.
    ...
    """
    sun_angle_correction = time_correction_for_solar_angle(time=ds.time)
...
def _solar_noon_pvlib(ds: xr.Dataset | xr.DataArray) -> xr.DataArray:
    """
    ...
    try:
        ...
        msg = (
            "_solar_noon_pvlib requires the pvlib library. "
...
def solar_noon(ds: xr.Dataset | xr.DataArray, method: Literal["pvlib", "astral", "internal"] = None) -> xr.DataArray:
    """
    ...
    """
    if method is None:
        if importlib.util.find_spec("pvlib"):
            method = "pvlib"
        elif importlib.util.find_spec("astral"):
            method = "astral"
        else:
            msg = (
                "Calculating solar noon with inaccurate internal method. "
                + "Ensure either pvlib or astral are installed."
            )
            warnings.warn(msg)
            method = "internal"
...
def interpolate_to_solar_noon(
    da: xr.Dataset | xr.DataArray, method: str = "interpolate", solar_method: str = None
    ...
    if isinstance(da, xr.Dataset):
        ds_out = da.map(interpolate_to_solar_noon, method=method, solar_method=solar_method)
        ds_out.attrs["history"] = update_history(f"{method}d to solar noon with {solar_method}")
        return ds_out
    ...
    days = da.time.resample(time="1D").first().dt.floor("D")
    other_coords = {d: da[d] for d in da.coords if d != "time"}
    ds_days = xr.Dataset({}, coords={"time": days, **other_coords})
    ...
    if method == "accumulate":
        ...
        # we need noon yesterday to accumulate between.
    ...
    if method == "interpolate":
        ...
        da_out.attrs["comments"] = f"Interpolated date to solar noons, calculated with {solar_method} library."
    elif method == "accumulate":
        da_out = da_out.assign_coords(noon=ds_days.noon, noon_yst=ds_days.noon_yst)
        # convert to accumulation units if necessary
        if da.attrs.get("units", None) is not None:
            da_out = to_agg_units(da_out, da, "sum")
        da_out.attrs["cell_methods"] = "time: sum (interval: 24 hr comment: since solar noon)"
        da_out.attrs["comments"] = f"Accumulated between solar noons, calculated with {solar_method} library."
    return da_out
