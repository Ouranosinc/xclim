...
* Faster backend for run length computations where the time is unchunked. ``_cumsum_reset`` was rewritten to include the fast track which is called with ``_cumsum_reset_np``. The slow track uses ``_cumsum_reset_xr`` which preserves all functionalities of the old ``_cumsum_reset``. (:pull:`2136`).
