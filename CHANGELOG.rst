...
* New arguments ``validity_ranges`` and ``hurs_cap_min`` in indicator ``universal_thermal_climate_index`` (UTCI). In accordance with recommendations from Bröde et al. (2012), default validity upper bounds were increased for ``tas - mrt`` (from 30°C to 70°C) and ``sfcWind`` (from 17 to 30.3 m/s) and capping is now enabled by default (``sfcWind`` to a minimum of 0.5 ms and of ``hurs``  to a minimum of 5 %). (:issue:`2400`, :pull:`2432`).
...
* ``xclim.indices._hydrology.days_with_snowpack`` was removed, use ``xclim.land.snw_days_above`` which has the same functionality. (:pull:`2267`).
* ``xclim.indices._hydrology.runoff_ratio`` order or argmuents has been changed, and `a` renamed to `area`. (:pull:`2267`).
