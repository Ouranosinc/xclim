...
Bug fixes
^^^^^^^^^
* One of the conditions in `atmos.cooling_degree_days_approximation` had the wrong computation and has been fixed: `(tasmax - tasmin)/4` is replaced by `(tasmax - thresh)/4` (:issue:`2272`).
