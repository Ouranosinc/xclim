...
* ``dry_spell_frequency`` now has a parameter `op: {"sum", "max"}` to choose if the threshold is less than the accumulated precipitation or the maximum daily precipitation.
