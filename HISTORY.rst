...
* Dependency ``pint`` now needs at least version 0.10.
...
* "degrees_north" and "degrees_east" (and their variants) are now considered independent units, so that ``pint`` and ``xc.core.units.ensure_cf_units`` don't convert them to "deg".
