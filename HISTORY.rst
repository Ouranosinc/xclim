...
* `xclim.core.calendar.percentile_doy` : `per` now accepts a sequence as well as a scalar and as such the output has a percentiles axis;
* `xclim.core.calendar.percentile_doy` : `per` argument is now expected to between 0-100 (not 0-1)
* `xclim.core.calendar.percentile_doy` : input data must have a daily (or coarser) time frequency
