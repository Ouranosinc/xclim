...
* Xclim's units registry and units formatting are now copied from `cf_xarray` with some negligeable tweaks. The exponent sign "^" is now never added in the ``units`` attribute. For example, square meters are given as "m2" instead of "m^2" by xclim, both are still accepted as input (:issue:`1010`, :pull:`1582`).
